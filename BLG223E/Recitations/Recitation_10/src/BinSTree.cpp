...
int BinSTree::leaf_count()
{
    cout << leaf_count(this->root) << endl;
}

int BinSTree::leaf_count(Node* r)
{
    if(r == NULL)
        return 0;
    else if(r->get_left() == NULL && r->get_right() == NULL)
        return 1;
    return leaf_count(r->get_left())+ leaf_count(r->get_right());
}

void BinSTree::depths()
{
    depths(this->root, 0);
}

void BinSTree::depths(Node* r, int depth)
{
    if(r!=NULL)
    {
        cout << r->get_data() <<"s depth=" << depth << endl;
        depths(r->get_left(),depth+1);
        depths(r->get_right(), depth+1);
    }
}

int BinSTree::heights()
{
    heights(this->root);
}

int BinSTree::heights(Node* r)
{
    if(r == NULL)
        return -1;
    int l_height = heights(r->get_left());
    int r_height = heights(r->get_right());
    int h = max(l_height,r_height)+1;
    //cout << r->get_data() << "'s height = " << h << endl;
    return h;
}
